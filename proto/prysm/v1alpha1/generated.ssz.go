...
// Hash: 46fa87bd94f02457c0644ae38b91942d56a4c2bced9aeee6063ddb7120cc1714
package eth
...
func (c *ColumnSidecar) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := len(c.Segments); size != 64 {
		err = ssz.ErrVectorLengthFn("--.Segments", size, 64)
		return
	}
	for ii := 0; ii < 64; ii++ {
	...
	if size := len(c.BlobKzgCommitments); size != 64 {
		err = ssz.ErrVectorLengthFn("--.BlobKzgCommitments", size, 64)
		return
	}
	for ii := 0; ii < 64; ii++ {
	...
	if size := len(c.SegmentKzgProofs); size != 64 {
		err = ssz.ErrVectorLengthFn("--.SegmentKzgProofs", size, 64)
		return
	}
	for ii := 0; ii < 64; ii++ {
	...
	// Field (5) 'CommitmentInclusionProof'
	if size := len(c.CommitmentInclusionProof); size != 17 {
		err = ssz.ErrVectorLengthFn("--.CommitmentInclusionProof", size, 17)
		return
	}
	for ii := 0; ii < 17; ii++ {
		if size := len(c.CommitmentInclusionProof[ii]); size != 32 {
			err = ssz.ErrBytesLengthFn("--.CommitmentInclusionProof[ii]", size, 32)
			return
		}
		dst = append(dst, c.CommitmentInclusionProof[ii]...)
	}
...
func (c *ColumnSidecar) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'Segments'
	c.Segments = make([][]byte, 64)
	for ii := 0; ii < 64; ii++ {
		if cap(c.Segments[ii]) == 0 {
			c.Segments[ii] = make([]byte, 0, len(buf[8:131080][ii*2048:(ii+1)*2048]))
		}
		c.Segments[ii] = append(c.Segments[ii], buf[8:131080][ii*2048:(ii+1)*2048]...)
	}

	// Field (2) 'BlobKzgCommitments'
	c.BlobKzgCommitments = make([][]byte, 64)
	for ii := 0; ii < 64; ii++ {
		if cap(c.BlobKzgCommitments[ii]) == 0 {
			c.BlobKzgCommitments[ii] = make([]byte, 0, len(buf[131080:134152][ii*48:(ii+1)*48]))
		}
		c.BlobKzgCommitments[ii] = append(c.BlobKzgCommitments[ii], buf[131080:134152][ii*48:(ii+1)*48]...)
	}

	// Field (3) 'SegmentKzgProofs'
	c.SegmentKzgProofs = make([][]byte, 64)
	for ii := 0; ii < 64; ii++ {
		if cap(c.SegmentKzgProofs[ii]) == 0 {
			c.SegmentKzgProofs[ii] = make([]byte, 0, len(buf[134152:137224][ii*48:(ii+1)*48]))
		}
		c.SegmentKzgProofs[ii] = append(c.SegmentKzgProofs[ii], buf[134152:137224][ii*48:(ii+1)*48]...)
	}
	...
	if err = c.SignedBlockHeader.UnmarshalSSZ(buf[137224:137432]); err != nil {
		return err
	}
	...
	for ii := 0; ii < 17; ii++ {
		if cap(c.CommitmentInclusionProof[ii]) == 0 {
			c.CommitmentInclusionProof[ii] = make([]byte, 0, len(buf[137432:137976][ii*32:(ii+1)*32]))
		}
		c.CommitmentInclusionProof[ii] = append(c.CommitmentInclusionProof[ii], buf[137432:137976][ii*32:(ii+1)*32]...)
	}
...
func (c *ColumnSidecar) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	...
	// Field (2) 'BlobKzgCommitments'
	{
		...
		subIndx := hh.Index()
		for _, i := range c.BlobKzgCommitments {
			if len(i) != 48 {
				err = ssz.ErrBytesLength
				return
			}
	...
	// Field (3) 'SegmentKzgProofs'
	{
		...
		subIndx := hh.Index()
		for _, i := range c.SegmentKzgProofs {
			if len(i) != 48 {
				err = ssz.ErrBytesLength
				return
			}
	...
	// Field (5) 'CommitmentInclusionProof'
	{
		...
		subIndx := hh.Index()
		for _, i := range c.CommitmentInclusionProof {
			if len(i) != 32 {
				err = ssz.ErrBytesLength
				return
			}
...
func (c *ColumnSidecars) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	dst = ssz.WriteOffset(dst, offset)
	offset += len(c.ColumnSidecars) * 137976
...
func (c *ColumnSidecars) UnmarshalSSZ(buf []byte) error {
	...
	{
		buf = tail[o0:]
		num, err := ssz.DivideInt2(len(buf), 137976, 128)
		if err != nil {
			return err
		}
		c.ColumnSidecars = make([]*ColumnSidecar, num)
		for ii := 0; ii < num; ii++ {
			if c.ColumnSidecars[ii] == nil {
				c.ColumnSidecars[ii] = new(ColumnSidecar)
			}
			if err = c.ColumnSidecars[ii].UnmarshalSSZ(buf[ii*137976 : (ii+1)*137976]); err != nil {
				return err
			}
		}
	}
...
func (c *ColumnSidecars) SizeSSZ() (size int) {
	...
	// Field (0) 'ColumnSidecars'
	size += len(c.ColumnSidecars) * 137976
