...
func (node *BeaconNode) Start(ctx context.Context) error {
	...
	args := []string{
		...
		fmt.Sprintf("--%s=%d", cmdshared.DHTTCPPort.Name, e2e.TestParams.Ports.PrysmBeaconNodeDHTPort+index),
		...
		fmt.Sprintf("--%s=%d", flags.ColumnBatchLimitBurstFactor.Name, 16),
		fmt.Sprintf("--%s=%d", flags.ColumnBatchLimit.Name, 256),
