...
type Blocker interface {
	...
	Columns(ctx context.Context, id string, indices []uint64) ([]*blocks.VerifiedROColumn, *core.RpcError)
}
...
//   - block exists, no commitment, 200 w/ empty list
//   - block exists, has commitments, inside retention period (greater of protocol- or user-specified) serve then w/ 200 unless we hit an error reading them.
//     we are technically not supposed to import a block to forkchoice unless we have the columns, so the nuance here is if we can't find the file and we are inside the protocol-defined retention period, then it's actually a 500.
//   - block exists, has commitments, outside retention period (greater of protocol- or user-specified) - ie just like block exists, no commitment
func (p *BeaconDbBlocker) Columns(ctx context.Context, id string, indices []uint64) ([]*blocks.VerifiedROColumn, *core.RpcError) {
	var root []byte
	switch id {
	case "genesis":
		return nil, &core.RpcError{Err: errors.New("columns are not supported for Phase 0 fork"), Reason: core.BadRequest}
	case "head":
		var err error
		root, err = p.ChainInfoFetcher.HeadRoot(ctx)
		if err != nil {
	...
	if len(indices) == 0 {
		...
		if err != nil {
			...
			return nil, &core.RpcError{Err: fmt.Errorf("could not retrieve column indices for root %#x", root), Reason: core.Internal}
		}
		for k, v := range m {
			if v {
				indices = append(indices, uint64(k))
	...
	// returns empty slice if there are no indices
	columns := make([]*blocks.VerifiedROColumn, len(indices))
	for i, index := range indices {
		vcolumn, err := p.ColumnStorage.Get(bytesutil.ToBytes32(root), index)
		if err != nil {
			log.WithFields(log.Fields{
				"blockRoot":   hexutil.Encode(root),
				"columnIndex": index,
			}).Error(errors.Wrapf(err, "could not retrieve column for block root %#x at index %d", root, index))
			return nil, &core.RpcError{Err: fmt.Errorf("could not retrieve blob for block root %#x at index %d", root, index), Reason: core.Internal}
		}
		columns[i] = &vcolumn
	}
